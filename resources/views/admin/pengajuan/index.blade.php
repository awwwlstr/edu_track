...
<div class="row g-3 mb-4">
    <div class="col-6 col-md-4">
        <div class="stat-card">
            ...
            <div class="info">
                <p>Menunggu</p>
                <h4>{{ $pengajuan->where('status', 'menunggu')->count() }}</h4>
            </div>
        </div>
    </div>
    <div class="col-6 col-md-4">
        <div class="stat-card">
            ...
            <div class="info">
                <p>Disetujui</p>
                <h4>{{ $pengajuan->where('status', 'disetujui')->count() }}</h4>
            </div>
        </div>
    </div>
    <div class="col-6 col-md-4">
        <div class="stat-card">
            ...
            <div class="info">
                <p>Ditolak</p>
                <h4>{{ $pengajuan->where('status', 'ditolak')->count() }}</h4>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <span class="font-mono" style="font-size:0.75rem;color:var(--text-light);background:var(--surface-2);border:1px solid var(--line);padding:2px 10px;border-radius:20px;">
                    {{ $pengajuan->where('status', 'menunggu')->count() }} data
                </span>
            </div>
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        @forelse($pengajuan->where('status', 'menunggu') as $key => $item)
                        <tr>
                            <td>
                            ...
                            <td>
                                @if($item->surat_keterangan)
                                    <a href="{{ asset('storage/surat/' . $item->surat_keterangan) }}" target="_blank"
                            ...
                            <td>
                                <div class="d-flex gap-2">
                                    {{-- Setujui --}}
                                    <form action="/admin/pengajuan/{{ $item->id_pengajuan }}/approve" method="POST" class="d-inline"
                                    ...
                                    <button type="button" title="Tolak"
                                            data-bs-toggle="modal" data-bs-target="#tolakModal{{ $item->id_pengajuan }}"
                                ...
                                {{-- Modal Tolak --}}
                                <div class="modal fade" id="tolakModal{{ $item->id_pengajuan }}" tabindex="-1">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <form action="/admin/pengajuan/{{ $item->id_pengajuan }}/reject" method="POST">
                                                @csrf
    ...
    <div class="tab-pane fade" id="disetujui" role="tabpanel">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <span class="font-mono" style="font-size:0.75rem;color:var(--text-light);background:var(--surface-2);border:1px solid var(--line);padding:2px 10px;border-radius:20px;">
                    {{ $pengajuan->where('status', 'disetujui')->count() }} data
                </span>
            </div>
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        @forelse($pengajuan->where('status', 'disetujui') as $key => $item)
                        <tr>
                            <td>
                                <span class="font-mono" style="color:var(--text-light);font-size:0.78rem;">{{ $key + 1 }}</span>
                            </td>
    ...
    <div class="tab-pane fade" id="ditolak" role="tabpanel">
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <span class="font-mono" style="font-size:0.75rem;color:var(--text-light);background:var(--surface-2);border:1px solid var(--line);padding:2px 10px;border-radius:20px;">
                    {{ $pengajuan->where('status', 'ditolak')->count() }} data
                </span>
            </div>
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        @forelse($pengajuan->where('status', 'ditolak') as $key => $item)
                        <tr>
                            <td>
                                <span class="font-mono" style="color:var(--text-light);font-size:0.78rem;">{{ $key + 1 }}</span>
                            </td>
